...
def main():
    ...
    x = 7.564  # East coordinate
    y = 45  # North coordinates
    n = 32
    collection = get_collection()
    start = "05/10/2017"
    end = "06/10/2017"
    start_time = time.mktime(datetime.datetime.strptime(start, "%d/%m" +
                                                        "/%Y").timetuple())
    ...
    array = ['#00008b', '#0f0f8f', '#191b93', '#212597', '#272f9b', '#2c389f',
             '#3141a2', '#364aa5', '#3a53a7', '#3d5caa', '#4165ab', '#446eac',
             '#4777ad', '#4a80ac', '#4d8aaa', '#5093a6', '#549da0', '#59a795',
             '#62b084', '#71b971', '#82c065', '#91c760', '#a0ce5e', '#aed55f',
             '#bcdc62', '#c9e366', '#d6ea6a', '#e2f170', '#eff876', '#fbff7c']
    for hour in fascie_orarie:
        ...
        for entry in l_coursor:
            d_origin = [int(round((entry["origin"][0] - x) / dx, 6)),
                        int(round((entry["origin"][1] - y) / dy, 6))]
            d_destination = [int(round((entry["destination"][0] - x) / dx, 6)),
                             int(round((entry["destination"][1] - y) / dy, 6))]
            if (0 < d_origin[0] < n and 0 < d_origin[1] < n
               and 0 < d_destination[0] < n
               and 0 < d_destination[1] < n):
                origin_grid = [x + (dx * d_origin[0]), y + (dy * d_origin[1])]
                destination_grid = [x + (dx*d_destination[0]),
                                    y + (dy*d_destination[1])]
        ...
        for pair in all_pair:
            ...
            d = {
                "type": "Feature",
                "properties": {"load": load, "maxload": maxload,
                               "color": array[c]},
                "geometry": {
                    "type": "LineString",
                    "coordinates": [
